...
struct SlowQueueBool(CollectionElement):
    var head: Int
    var tail: Int
    var capacity: Int
    var size: Int
    ...
    fn __init__(inout self: Self, capacity: Int):
        ...
        self.capacity = capacity
        self.size = 0
        self.data = DynamicVector[Bool](capacity)
    ...
    fn push_back(inout self: Self, data: Bool):
        if self.tail == self.capacity - 1:
            if self.size * 2 > self.capacity:
                # resize buffer
                let tmp = self.data
            ...
            #self.data.__setitem__(self.tail, data)
            self.tail += 1
        self.size += 1

        print(self.data[self.tail - 1])

    fn front(inout self: Self) raises -> Bool:
        if self.size <= 0:
            raise Error("Queue is empty.")
